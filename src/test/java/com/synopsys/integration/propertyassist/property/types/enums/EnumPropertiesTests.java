 ...
 * under the License.
 */
package com.synopsys.integration.propertyassist.property.types.enums;
...
public class EnumPropertiesTests {
    ...
    public void testExampleValues() {
        ...
        Assertions.assertEquals(Arrays.asList("THING", "ANOTHER", "THIRD"), property.listExampleValues());
    }
    ...
    public void testList() throws InvalidPropertyException {
        final EnumListProperty<Example> property = new EnumListProperty<>("enum.list", Arrays.asList(Example.THIRD), Example.class);
        final PropertyConfiguration config = configOf(Pair.of("enum.list", "ANOTHER,THING"));
        Assertions.assertEquals(Arrays.asList(Example.ANOTHER, Example.THING), config.getValue(property));
